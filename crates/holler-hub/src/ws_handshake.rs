//! The raw WebSocket **server-side opening handshake** over an
//! already-accepted loopback `TcpStream`, split out of `serve.rs` (issue
//! #184) once that file's own growth (the connection-hygiene accept-path
//! additions: the pre-auth semaphore, the lockout check, the pre-auth
//! timeout wrap) pushed it past the workspace's 900-line build guard
//! (`scripts/lint.sh` check 4) — a pure relocation of the handshake mechanics
//! only, no behavior change of its own.
//!
//! tokio-tungstenite ships only a *client* async handshake (`connect_async`);
//! its `handshake::server` machine is blocking (`std::io::Read`/`Write`),
//! which a tokio `TcpStream` does not implement. So we do the handshake by
//! hand on the async socket: read the upgrade `GET` (until the blank line),
//! parse it, compute the `Sec-WebSocket-Accept` via tungstenite's own
//! (tested) `create_response`, write the `101` back with `AsyncWrite`, and
//! hand the socket — plus any bytes we over-read — to `from_partially_read`
//! so no frame data is lost.
...
/// `WebSocketStream`, configured with `ws_config` (issue #184's max frame/
/// message size hygiene limit). A client that sends a non-WebSocket `GET`
/// (or nothing) is dropped (returns `None`).
pub(crate) async fn server_handshake(mut stream: TcpStream, ws_config: WebSocketConfig) -> Option<WebSocketStream<TcpStream>> {
    ...
    Some(WebSocketStream::from_partially_read(stream, rest.to_vec(), Role::Server, Some(ws_config)).await)
}
...
fn parse_upgrade_request(head: &[u8]) -> Option<WsRequest> {
    ...
    let mut builder = http::Request::builder().method(method).uri(path).version(http::Version::HTTP_11);
