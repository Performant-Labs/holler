...
// The one-shot CLI command runners (`roster`/`hub status`/`body status`/
// `say`/`interrupt`/`answer`/`wait`) — split out (issue #142) to keep this
// file under the 900-line build guard. Re-exported so every existing
// `support::roster_json(...)` etc. call site is unchanged.
mod cmds;
// Token onboarding (`mint_token`/`hub_pubkey`/`join`), split out for the same
// reason and re-exported so every `support::mint_token(...)` call site is unchanged.
mod onboard;
#[allow(unused_imports)] // #450
pub use onboard::{hub_pubkey, join, mint_token};
// Each integration-test binary only calls a subset of these (one `mod
// support;` per test target, so an unused re-export here is a real, expected
...
pub struct Hub {
    pub port: u16,
    child: Child,
    /// Every stderr line the hub has emitted so far (issue #450's tests
    /// assert on the default-level log). Filled by the drainer thread.
...
impl Hub {
    ...
    pub fn start_with_env(state: &StateDir, envs: &[(&str, &str)]) -> Hub {
        ...
        let port = {
            ...
            let sink = std::sync::Arc::clone(&log_lines);
    ...
    pub fn log_text(&self) -> String {
        self.log_lines.lock().map(|l| l.join("\n")).unwrap_or_default()
    }
