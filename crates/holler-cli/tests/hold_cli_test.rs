...
/// `holler say ...`, retried while the hub answers `session_busy`. A busy
/// refusal delivers nothing, so a retry cannot double-deliver; it is needed
/// because the hub's presence cache can trail the roster row for a moment
/// after a turn ends. Held refusals are not retried.
...
fn hold_roster_say_refused_release_say_works_text() {
    ...
    let said = say_retrying(&state, &["say", "b/alpha", "hi"]);
...
fn hold_release_roster_and_say_json_shapes_are_pinned() {
    ...
    assert_eq!(say_retrying(&state, &["say", "b/alpha", "hi", "--json"]).code, 0);
    body.stop(&state, Duration::from_secs(10));
}
...
fn a_held_session_does_not_affect_another_over_the_cli() {
    ...
    assert_eq!(say_retrying(&state, &["say", "b/beta", "hi"]).code, 0);
    body.stop(&state, Duration::from_secs(10));
}
