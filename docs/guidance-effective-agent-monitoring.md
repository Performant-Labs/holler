...
- Spawn-mode (ACP) sessions are answered the same way, with the same command
  (`holler answer <session> <choice>`): a 0-based option index, an option's key
  or label, or on a permission prompt `once`, `always` or `reject` (chosen by
  the option's ACP kind). Multi-question requests where the question count
  itself is unknown ahead of time still require reading `/question` directly
  to discover the shape before answering.
...
- **Codex does surface permission requests, and Holler reports them as
  ...
  Codex what to do differently`. This works over ACP v1, which Codex negotiates.
...
- **Answer with `holler answer <session> <choice>`.** `0` approves once and `2`
  rejects (the option order the adapter sent); `once`, `always` and `reject`
  also work, resolved by the option's ACP kind, and an option can be selected by
  its label even when the label contains a comma, such as the reject option
  ([#476](https://github.com/Performant-Labs/holler/issues/476),
