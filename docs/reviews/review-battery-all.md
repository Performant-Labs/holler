...
| $WORKFLOW_ROOT/workflow/skills/thermo-nuclear-code-quality-review/SKILL.md | skill | Shape | canonical file exists |
| $WORKFLOW_ROOT/workflow/skills/thermo-nuclear-review/SKILL.md | skill | Truth+Safety | same |
| $WORKFLOW_ROOT/workflow/skills/thermos/SKILL.md | skill | optional 1+2 together | same; missing is WARN, not FAIL — run 1 and 2 separately |
| $WORKFLOW_ROOT/workflow/skills/code-review/SKILL.md | skill | Spec | same |
| $WORKFLOW_ROOT/workflow/skills/security-review/SKILL.md | skill | Exploitability | same |
| $WORKFLOW_ROOT/workflow/skills/tests-as-evidence/SKILL.md | skill | Tests | same |
...
| ./.agents/skills, ./.claude/skills, ./.grok/skills in this repo | skill | must be **absent** | **PASS if missing.** Those trees were moved to workflow repo. Do not FAIL thermos, code-review, security-review, tests-as-evidence (or any sibling) for not existing here. WARN only if the directory is present (vendored copy). |

A skill is installed if the **workflow repo** SKILL.md exists. Stubs are user-global pointers (`~/.agents/skills`, not `./.agents/skills`). FAIL the canonical row if workflow repo is missing that skill. A missing `./.agents/skills/<name>/SKILL.md` is expected and is not a fail.
