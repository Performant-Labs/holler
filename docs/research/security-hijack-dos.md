...
> code-level findings below (`wire/mod.rs`, `wire/connection.rs`, `wire/registry.rs`,
> `token/mod.rs`) were verified against `holler-server`'s Rust source at the time — they are
> **not re-verified against current `holler` code** and should not be assumed to still hold; the
> reasoning and industry-precedent research (SSH, fail2ban, OWASP, the vendor survey) remains
> directly applicable to hardening `holler`'s own hub.
...
(b) **revocability**: `v1 holler token delete <id>` already revokes a bound credential outright
