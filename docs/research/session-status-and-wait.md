...
> "server" below is today's hub, "client" is today's body, the legacy `holler-server` verbs (`wait`, `say`, `roster`, ...) are today's hub-only
> top-level verbs, and `session_blocked` / `session_status` are v1 wire frames, not v2
> method names (see [`../protocol/v2.md`](../protocol/v2.md) for the v2 surface). **What was built
> from it:** the blocking, poll-free `wait` verb with `--until` / `--after TURN_ID` edge-triggering
