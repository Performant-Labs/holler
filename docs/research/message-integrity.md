...
> `src/proto/mod.rs` code-level findings are **not re-verified against current `holler` code**; the
> integrity/ack reasoning (TCP/TLS guarantees, MQTT QoS precedent, the `interrupt`-needs-an-ack
> conclusion) remains directly applicable.
